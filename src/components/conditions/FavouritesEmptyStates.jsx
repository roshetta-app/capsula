 ...
 *
 * Empty-state visual pass 3 (this session) — per feedback:
 *  - Description sentence ("Save conditions/drugs for quick access...")
 *    is gone; the heart-tap tip below is now the only subtitle line,
 *    doing double duty as both explanation and mechanism hint. Small
 *    inline Heart icon (lucide-react, var(--color-favourite) — the same
 *    accent token FavouritesHero.jsx's own heart badge uses) added back
 *    for this one line only; FAV_ACCENT itself stays removed (was only
 *    ever the old Heart-in-circle placeholder graphic's color, dropped
 *    with it).
 *  - The sign-in divider + outlined button are gone entirely — per
 *    feedback they were competing with Browse for attention. Replaced
 *    with a single quiet text line ("Have an account? Sign in", "Sign
 *    in" in accent color) — no border, no fill, no divider rule. Still
 *    calls the same handleGoogleSignIn/busy/error-message logic as
 *    before, just presented as a plain tappable line instead of a boxed
...
import { SearchX, ArrowRight, Heart } from 'lucide-react'
      ...
      <div style={{
        ...
        {showSignIn && (
          <div style={{
            ...
            {/* Replaces the earlier divider + outlined Google button —
                per feedback that pairing competed with Browse for
                attention. Plain text, no border/fill, "Sign in" the only
                colored word — reads as clearly secondary without a second
                boxed action on screen. Still the same
                handleGoogleSignIn/busy/error-message logic as before. */}
            <button
            ...
            >
              Have an account?{' '}
              <span style={{
                color:      busy ? 'var(--color-text-tertiary)' : 'var(--color-accent)',
                fontWeight: 600,
              }}>
