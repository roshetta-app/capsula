...
export default function ManageStock({ drugs, stockMap, onToggle, onSetAll, onBack }) {
  ...
  const sortedDrugs = useMemo(() => {
    const inStock  = drugs.filter(d =>  stockMap[d.id]).sort((a, b) => a.genericName.localeCompare(b.genericName))
    const outStock = drugs.filter(d => !stockMap[d.id]).sort((a, b) => a.genericName.localeCompare(b.genericName))
    return [...inStock, ...outStock]
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []) // freeze order at mount
  ...
  const visibleDrugs = useMemo(() => {
    ...
    return sortedDrugs.filter(d =>
      ...
      d.brands?.some(b => b.name.toLowerCase().includes(lower))  // FlatDrug: brands[]
    )
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 'var(--space-4)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-1)' }}>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--color-text-primary)', margin: 0, letterSpacing: '-0.3px' }}>
            Manage Stock
          </h1>
          <button
            ...
            style={{
              ...
              backgroundColor: allVisibleInStock ? 'var(--color-instock-bg)' : 'var(--color-surface)',
              color: allVisibleInStock ? 'var(--color-instock)' : 'var(--color-text-secondary)',
        ...
        <div style={{ fontSize: 13, color: 'var(--color-text-tertiary)', fontFamily: 'var(--font-mono)' }}>
      ...
      {visibleDrugs.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 'var(--space-10) var(--space-4)', color: 'var(--color-text-tertiary)', fontSize: 14 }}>
        ...
        <div style={{
          ...
          {visibleDrugs.map((drug, index) => {
            ...
            const brandNames = drug.brands?.map(b => b.name) ?? []  // FlatDrug
            ...
            return (
              <div
                ...
                style={{
                  ...
                  gap: 'var(--space-3)', transition: 'background-color 0.1s ease',
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 'var(--radius-full)', border: 'none', cursor: 'pointer', padding: 3,
